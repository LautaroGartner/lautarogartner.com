...
function authorUrlLabel(authorUrl: string): string {
  try {
...
function authorSocialLabel(site: SiteDefinition): string {
  if (!site.authorUrl) {
    return site.author ?? AUTHOR_USERNAME;
  }

  try {
    const url = new URL(site.authorUrl);
    ...
    // Fall back to the configured author below.
  }
...
function renderHeader(
  ...
  const pageLinks = site.pages
    ...
    .join("");
  const authorLink = site.authorUrl
    ? `
          <a href="${escapeHtml(site.authorUrl)}" rel="me">${escapeHtml(site.followLabel ?? "Follow")}</a>`
    : "";
  const languageLink = normalizedPath === "/web"
    ? `
          <a href="/web/es" lang="es">Ver en español</a>`
    : normalizedPath === "/web/es"
      ? `
...
function renderFooter(site: SiteDefinition, options: { generatedBy?: boolean } = {}): string {
  const generatedBy =
  ...
  const author = site.author ?? AUTHOR_NAME;
  const authorText = site.authorUrl
    ? `${escapeHtml(author)} <a href="${escapeHtml(site.authorUrl)}" rel="me">${escapeHtml(authorSocialLabel(site))}</a>`
    : escapeHtml(author);
  ...
  return `<footer>
        ...
        ${generatedBy}
        <a href="${escapeHtml(site.sourceUrl ?? SOURCE_URL)}">Source</a>
      </footer>`;
}
...
export function generateSitePage(
  ...
  const structuredData = {
    ...
    inLanguage: page.language ?? siteLanguage(site),
    author: site.author
      ? {
          "@type": "Person",
          name: site.author,
